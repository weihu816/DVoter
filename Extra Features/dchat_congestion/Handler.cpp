...
string Handler::process(Address & from_addr, string recv_msg) {
    ...
    if (recv_msg.find("#") == 0) { // Start with a #
        ...
        // for traffic
        if(strcmp(msg_type, D_FAST) == 0 || strcmp(msg_type, D_SLOW) == 0 ) {
#ifdef DEBUGLOG2
            std::cout << "traffic: " << msg_type <<  std::endl;
#endif
            node->updataSleepInt(msg_type);
#ifdef DEBUGLOG2
            std::cout << "traffic: finished updating." <<  std::endl;
            return "OK";
#endif
        } else if (strcmp(msg_type, D_CHAT) == 0) {
            ...
            if (nodeMember->isLeader()) { // Only leader can multicast messages
                ...
                // record the number seen
                auto & message_counter_table = node->message_counter_table;
                if (message_counter_table.find(address_port) == message_counter_table.end()) {
                    message_counter_table[address_port] = 0;
                }
                message_counter_table[address_port] += 1;
