 ...
 * FUNCTION NAME: peek
 *
 * DESCRIPTION: get top member of queue without popping
 */
T holdback_queue::peek()  {
    std::unique_lock<std::mutex> mlock(d_mutex);
    T peek_pair = d_queue.top();
    return peek_pair;
}
...
int holdback_queue::getHoldBackQueueSize() {
    return (int)d_queue.size();
}

void holdback_queue::setSequenceSeen(int i) {
    sequence_seen = i;
}

void holdback_queue::setSequenceNext(int i) {
    sequence_next = i;
}
